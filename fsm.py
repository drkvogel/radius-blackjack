...
def transition_new_group(fsm_obj):
    ...
    fsm_obj.current_group = RuleGroup(fsm_obj.current_group, fsm_obj.group_current_level, None)
...
FSM_MAP = (
    ...
    {'src': S_NEW_GROUP,    'dst': S_PRE,       'condition': "[A-Za-z|+|-|\d]", 'callback': T_APPEND_CHAR_PRE},     # 1
    {'src': S_PRE,          'dst': S_PRE,       'condition': "[A-Za-z|+|-|\d]", 'callback': T_APPEND_CHAR_PRE},     # 2
    {'src': S_PRE,          'dst': S_SUBJ,      'condition': r"\(",             'callback': T_SKIP},                # 3
    {'src': S_SUBJ,         'dst': S_SUBJ,      'condition': "[^\)]",           'callback': T_APPEND_CHAR_SUBJ},    # 4
    {'src': S_SUBJ,         'dst': S_END_RULE,  'condition': "\)",              'callback': T_END_RULE},            # 5
    {'src': S_END_RULE,     'dst': S_END_GROUP, 'condition': "\)",              'callback': T_END_GROUP},           # 6
    {'src': S_END_RULE,     'dst': S_OP,        'condition': "[\&|\|]",         'callback': T_ADD_OP_NEW_RULE},     # 7
    {'src': S_END_GROUP,    'dst': S_OP,        'condition': "[\&|\|]",         'callback': T_ADD_GROUP_OP},        # 8
    {'src': S_OP,           'dst': S_NEW_GROUP, 'condition': "\(",              'callback': T_NEW_GROUP},           # 9
    {'src': S_OP,           'dst': S_PRE,       'condition': "[A-Za-z|+|-|\d]", 'callback': T_APPEND_CHAR_PRE},     # 10
    {'src': S_SUBJ,         'dst': S_END_RULE,  'condition': "\)",              'callback': T_END_RULE})            # 11
...
class Rule_Parse_FSM:
    ...
    def process_next(self, achar):
        ...
        for transition in FSM_MAP:
            ...
            if transition['src'] == self.current_state:
    ...
    def update_state(self, new_state, callback):
        print("{} -> {} : {}".format(self.current_char, self.current_state, new_state))
